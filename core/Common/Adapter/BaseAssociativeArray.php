...
abstract class BaseAssociativeArray implements AssociativeArrayInterface
{
     ...
     * @return void
     *
     * @throws EmptyAdapterDataException if mandatory field is not set or is null
     * @throws InvalidAdapterDataTypeException if the type of given value doesn't match the type specified in getFieldTypes() method
     */
    protected function validateFields(array $data): void
    {
        foreach ($this->getMandatoryFields() as $field) {
            if (!array_key_exists($field, $data)) {
                throw new EmptyAdapterDataException(
        ...
        foreach ($data as $key => $value) {
            ...
            if (!$this->isValidFieldType($key, gettype($value))) {
                throw new InvalidAdapterDataTypeException(
